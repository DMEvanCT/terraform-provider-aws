...
~> In order to prevent persistent differences when using this resource, the parent cluster should include an `ignore_changes` lifecycle configuration containing the `snapshot_copy` argument. This will be necessary only until the deprecated argument is removed in a future major version.
...
resource "aws_redshift_cluster" "example" {
  ...
  # snapshot_copy argument is removed in a future major version.
  lifecycle {
    ignore_changes = [snapshot_copy]
  }
}
